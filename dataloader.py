...
from data.utils import load_cpf_data
from data_preprocess import *


DGL_data = []
CPF_data = ["cora", "citeseer", "pubmed","amazon-com", "amazon-photo", "coauthor-cs", "coauthor-phy"]
...
def load_data(dataset, **kwargs):
    if dataset in DGL_data:
        ...
        return load_cpf_data(dataset, dataset_path="./data/npz", seed=kwargs["seed"])
...
def load_dgl_data(dataset):
    ...
    train_mask = graph.ndata['train_mask'].nonzero(as_tuple=True)[0]
    val_mask = graph.ndata['val_mask'].nonzero(as_tuple=True)[0]
    test_mask = graph.ndata['test_mask'].nonzero(as_tuple=True)[0]
