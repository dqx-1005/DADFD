...
class GAT(nn.Module):
    ...
    def forward(self, inputs):
        ...
        for l in range(self.num_layers):
            ...
            h, att = self.layers[l](self.g, h)
            h = h.flatten(1)
            middle.append(h)
            if l == self.num_layers - 1:
                penultimate_layer = h
        # output projection
        logits, att = self.layers[-1](self.g, h)
        logits = logits.mean(1)
        middle.append(logits)
        penultimate_layer = middle[-2]
        return logits, att, middle,penultimate_layer
...
class GATConv(nn.Module):
    ...
    def forward(self, graph, feat):
        ...
        with graph.local_scope():
            ...
            # We decompose the weight0 vector a mentioned in the paper into
            # [a_l || a_r], then
...
class ogb_GAT(nn.Module):
    def __init__(
        ...
        for i in range(n_layers):
            ...
            # in_channels = n_heads if i > 0 else 1-1
